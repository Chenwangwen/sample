...
function describePerson(person) {
  let description = `${person.name} is a ${person.age}-year-old `;
  if (person.gender === "m") {
    description += "man. ";
  } else {
    description += "woman. ";
  ...
  if (person.hobby) {
    description += person.gender === "m" ? "He" : "She";
    description += ` likes ${person.hobby}!`;
  } else {
    description += person.gender === "m" ? "He" : "She";
    description += " has no hobbies.";
  }
...
function analyzeMarks(marks) {
  ...
  const average = sum / Object.keys(marks).length; 
  return [min, max, parseFloat(average.toFixed(2))];
}
...
function check(arr, target) {
  ...
  for (let i = 0; i < arr.length; i++) {
    ...
    if (indicesMap[complement] !== undefined) {
      return [indicesMap[complement], i].sort((a, b) => a - b); 
    }
